 ...
 *
 */
fun countSubstringEntries(line: String, word: String): Int {
    val allSubstrings = line.toLowerCase().windowed(word.length, 1)
    return if (allSubstrings.count { it == word } != 0) allSubstrings.count { it == word }
    else 0
}

fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val result = mutableMapOf<String, Int>()
    val substringsSet = substrings.toMutableSet()
    for (line in File(inputName).readLines()) {
        for (word in substringsSet) {
            val line = line.toLowerCase()
            result[word] = result.getOrDefault(word, 0) + countSubstringEntries(line, word.toLowerCase())
        }
    }
    return result
}
    ...
    File(outputName).bufferedWriter().use {
        for (line in File(inputName).readLines()) {
            ...
            for (i in 0 until line.length - 1) {
                if (line[i].toString().contains("""[ЖжЧчШшЩщ]""".toRegex())) {
                    when (line[i + 1]) {
                        'Ы' -> it.write("И")
                        'ы' -> it.write("и")
                        'Я' -> it.write("А")
                    ...
                    if (line[i + 1].toString().contains("""[ЫыЯяЮю]""".toRegex())) continue
                }
    ...
    for (line in File(inputName).readLines()) {
        for (i in line) {
            if (dictionary.containsKey(i.toLowerCase()) || dictionary.containsKey(i.toUpperCase())) {
                ...
                } else writer.write(dictionary[i].toString().toLowerCase())
                continue
            }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    if (inputName.isNotEmpty()) {
        ...
        writer.write(result[result.keys.max()])
    } else {
        writer.write("")
        writer.close()
    }
