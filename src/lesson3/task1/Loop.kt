...
fun squareBetweenExists(m: Int, n: Int): Boolean {
    return (sqr(floor(sqrt(m.toDouble()))).toInt() == m || sqr(floor(sqrt(m.toDouble()))).toInt() == m)
            || (sqrt(n.toDouble()).toInt() - sqrt(m.toDouble()).toInt() > 0) //зачем он так странно отформатировал
}
